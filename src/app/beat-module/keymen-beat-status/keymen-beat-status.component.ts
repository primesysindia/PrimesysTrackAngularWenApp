...
export class KeymenBeatStatusComponent implements OnInit {
  ...
  constructor(private beatService: BeatServicesService,private pdfService: PdfServiceService,
    public dialog: MatDialog, private excelServ: BeatExcelService,private datePipe: DatePipe) { }
  ...
  downloadFile(){
    this.excelServ.generateBeatReportExcel(this.responseData,this.tableHeader)
  }

  generatePdf() {
    // console.log(pdfMake);
    const documentDefinition = this.getDocumentDefinition();
    // pdfMake.createPdf(documentDefinition).download('Keymen Beats');
    pdfMake.createPdf(documentDefinition).open();
  }

  getDocumentDefinition() {
    return {
      // watermark:{ text: 'Primesys Technologies', color: 'grey', opacity: 0.3, bold: false, italics: false, fontSize: 50 },
  ...
  getBeatObject(beats: getApprovalBeats[]) {
    return {
      table: {
        widths: ['auto', '*', '*', '*','*','*'],
         headerRows: 1,
        body: [
          ...
          ...beats.map(beat => {
            ...
            return ([
              beat.Devicename, 
                beat.KmStart, beat.KmEnd, beat.SectionName,this.datePipe.transform(beat.ApprovedDate, 'MMM d, y'), beat.isApprove == true ? 'Approved' : 'Not Approved'
            ]); 
          })
        ]
      }
